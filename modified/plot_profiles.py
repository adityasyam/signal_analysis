...
def plot_profiles_split_channels(profiles, n_channels, maxval, minval):
    channel_width = profiles.shape[0] // n_channels

    profiles_img = np.zeros(((channel_width + 5) * n_channels, profiles.shape[1], 3))

    for n in range(n_channels):
        channel_profiles = profiles[n * channel_width: (n + 1) * channel_width]
        profiles_img[n * (channel_width + 5): (n + 1) * (channel_width + 5) - 5, :, :] = plot_profiles(channel_profiles, maxval, minval)
...
if __name__ == '__main__':
    ...
    plot_profiles_file(args.profiles_file, args.diff, args.n_channels, args.maxval, args.maxdiffval, args.mindiffval)